...
//             try {
//                 const response = await axiosInstance.get("/api/order"); // Replace with your API URL
...
//                 // Successfully updated the status, update it in the UI state
                                    ...
                                    <TableCell colSpan={7} sx={{paddingBottom: 0, paddingTop: 0}}>
                                        ...
                                        <Collapse in={expandedOrderId === order._id} timeout="auto" unmountOnExit>
                                            <Box sx={{margin: 2, padding: 2, background: "#f5f5f5", borderRadius: 2}}>
                                                ...
                                                <Typography><strong>Name:</strong> {order.user_id?.first_name} {order.user_id?.last_name}
                                                </Typography>
                                                <Typography><strong>Email:</strong> {order.user_id?.email}
                                                </Typography>
                                                <Typography>
                                                    <strong>Address:</strong> {order.user_id?.address_details?.address_1}, {order.user_id?.address_details?.address_2}, {order.user_id?.address_details?.city}, {order.user_id?.address_details?.state}, {order.user_id?.address_details?.country}, {order.user_id?.address_details?.zipcode}
                                                </Typography>
                                                {/*<Typography><strong>Payment*/}
                                                {/*    Method:</strong> {order.payment_method || "N/A"}</Typography>*/}
                                                <Typography><strong>Order
                                                    Date:</strong> {new Date(order.createdAt).toLocaleString()}
                                                </Typography>
                                            </Box>
                                        </Collapse>
                                    </TableCell>
